...
const QuizLetter = () => {
    ...
    const fetchQuizData = async () => {
      try {
        ...
        const filteredQuizzes = data.filter(
          (quiz) => quiz.quiz_Letter === letter.toUpperCase().charCodeAt(0) - 64
        );
        ...
        if (filteredQuizzes.length > 0) {
          ...
          setQuizData(null);
          setError(`No quizzes available for letter ${letter.toUpperCase()}`);
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (quizData) {
      ...
      const fetchLetterProgress = async () => {
        try {
          const token = localStorage.getItem("access_token");
          const response = await fetch(
            `http://127.0.0.1:8000/api/user-letter-progress/${letter.toUpperCase()}/`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
  ...
  const nextLetter = letter.toLowerCase().charCodeAt(0) + 1;
  let path;
  if (nextLetter > 122) {
    path = `/letter/A`;
  } else {
    path = `/letter/${String.fromCharCode(nextLetter)}`;
  }
  if (error) {
    ...
    <>
      {!quizData ? (
        ...
        <div className={styles.container}>
          <h1>Quiz for Letter {letter.toUpperCase()}</h1>
          {result ? (
            <>
              <h2>
              ...
              {pass ? (
                <Link to={path}>
                  <button className={styles.button}>Next Letter</button>
                </Link>
              ) : (
